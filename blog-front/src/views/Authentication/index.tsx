...
export default function Authentication() {
  ...
  const SignInCard = () => {
    ...
    const singInResponse = (responseBody: SignInResponseDto | ResponseDto | null) => {
      if (!responseBody) {
        alert('네트워크 이상입니다.');
        return;
      }
    ...
    const onSignInButtonClickHandler = () => {
      ...
      signInRequest(requestBody).then(singInResponse)
    }
        ...
        <div className='auth-card'>
          <div className='auth-card-box'>
            <div className='auth-card-top'>
              ...
              <InputBox ref={emailRef} label='이메일주소' type='text' placeholder='이메일 주소를 입력해주세요.'
            ...
            <div className='auth-card-bottom'>
              ...
              <div className='auth-description-box'>
                <div className='auth-description'>{'신규 사용자 이신가요?'}<span
                    ...
                    onClick={onSignUpLinkClickHandler}>{'회원가입'}</span></div>
              </div>
            </div>
          </div>
        </div>);
  ...
  const SignUpCard = () => {
    ...
    const onAddressDetailHandler = (event: ChangeEvent<HTMLInputElement>) => {
      const {value} = event.target;
...
//  event handler : 개인 정보 동의 체크박스 클릭 이벤트 처리
    ...
    const onAddressButtonClickHander = () => {
      open({onComplete});
    }
    ...
    const onNextButtonClickHandler = () => {
      ...
      if (!emailPattern || !isCheckedPassword || !isEqualPassword) return;
    ...
    const onSignUpButtonClickHandler = () => {
      ...
      if (!emailPattern || !isCheckedPassword || !isEqualPassword) {
        return setPage(1);
      }
    ...
    return (<div className='auth-card'>
      <div className='auth-card-box'>
          ...
          {page === 1 && (
              <>
                ...
                <InputBox ref={passwordRef} label='비밀번호*' type={passwordType}
                          ...
                          onChange={onPasswordChangeHandler} error={isPasswordError}
          ...
          {page === 2 && (
              <>
                ...
                <InputBox ref={telNumberRef} label='핸드폰 번호*' type='text'
                          placeholder='핸드폰 번호을 입력해주세요.' value={telNumber}
                          onChange={onTelNumberChangeHandler} error={isTelNumberError}
                          message={TelNumberErrorMessage} onKeyDown={onAddressKeyDownHandler}/>
                <InputBox ref={addressRef} label='주소*' type='text' placeholder='우편번호 찾기'
                          ...
                          message={addressErrorMessage} icon='search-light-icon'
                          onButtonClick={onAddressButtonClickHander}
                ...
                <InputBox ref={addressDetailRef} label='상세 주소' type='text'
                          ...
                          onChange={onAddressDetailHandler} error={false}
        ...
        <div className='auth-card-bottom'>
          {page === 1 && (
              <div className='black-large-full-button'
                   onClick={onNextButtonClickHandler}>{'다음 단계'}</div>
          )}
              ...
              <>
                <div className='auth-consent-box' onClick={onAgreedPersonalClickHandler}>
                  <div className='auth-check-box'>
                    <div
                        className={`icon ${agreedPersonal ? 'check-round-fill-icon' : 'check-round-light-icon'}`}></div>
                  </div>
                  <div
                ...
                <div className='black-large-full-button'
                     onClick={onSignUpButtonClickHandler}>{'회원 가입'}</div>
              </>
          )}

          <div className='auth-description-box'>
            <div className='auth-description'>{'이미 계정이 있으신가요'}<span
                className='auth-description-link' onClick={onSignInLinkClickHandler}>{'로그인'}</span>
            </div>
          </div>
        </div>
      </div>
          ...
          <div className='auth-jumbotron-box'>
            <div className='auth-jumbotron-content'>
              ...
              <div className='auth-jumbotron-text-box'>
                ...
                <div className='auth-jombotron-text'>{'김영준 보드입니다.'}</div>
              </div>
            </div>
          </div>
