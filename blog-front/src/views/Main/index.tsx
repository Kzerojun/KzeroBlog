...
import {getLatestBoardListRqueust, getPopularListRequest, getTop3BoardListRequest} from 'apis'
...
export default function Main() {
    ...
    return (
        <div id='main-top-wrapper'>
          <div className='main-top-container'>
            <div className='main-top-title'>{'KzeroJun Board에서\n 다양한 이야기를 나눠보세요'}</div>
  ...
  const MainBottom = () => {
    ...
    const getPopularListResopnse = (responseBody: GetPopularListResponseDto | ResponseDto | null) => {
      if (!responseBody) return;
      const {code} = responseBody;
    ...
    //  effect : 첫 마운트 시 실행될 함수  //
    useEffect(() => {
      getLatestBoardListRqueust().then(getLatestBoardListResponse);
      getPopularListRequest().then(getPopularListResopnse);
    }, []);
                ...
                <div className='main-bottom-popular-card'>
                  <div className='main-bottom-popular-card-container'>
                    ...
                    <div className='main-bottom-popular-card-contents'>
                      {popularWordList.map(word => <div className='word-badge'
                                                        onClick={() => onPopularWordClickHandler(word)}>{word}</div>)}
                    </div>
                  </div>
                </div>
              </div>
